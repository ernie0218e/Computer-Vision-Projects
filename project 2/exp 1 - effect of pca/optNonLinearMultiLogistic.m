...
%         for n = 1:N
%            if W(n, i) == 1
%                phi_zero_g(n, :) = phi_zero_g(n, :) + (y(n) - 1);
%                phi_g(n, :) = phi_g(n, :) + (y(n) - 1)*atan(zeta(n, :)*x(i, :)');
%                zeta_g(n, :) = zeta_g(n, :) ...
%                    + (y(n) - 1)*phi(n)*(1/(1+(zeta(n, :)*x(i, :)')^2)).*x(i, :);
%            else
%                phi_zero_g(n, :) = phi_zero_g(n, :) + (y(n));
%                phi_g(n, :) = phi_g(n, :) + (y(n))*atan(zeta(n, :)*x(i, :)');
%                zeta_g(n, :) = zeta_g(n, :) ...
%                    + (y(n))*phi(n)*(1/(1+(zeta(n, :)*x(i, :)')^2)).*x(i, :);
%            end
%         end
%         
%     end
